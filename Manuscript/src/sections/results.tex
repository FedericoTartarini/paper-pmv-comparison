...
% todo: remove the gridlines or marker them very short; remove outliers or make the line very thin; keep one decimal for clo values; remove decimal point for RH; annotation font should be the same as the label and same size, change only the color; You can use black or blue (HEX: 003262, RGB (0, 50, 98)) for the chart; one decimal for met not two; the chart is a bit dense of information
\begin{figure*}[htb!]
    \centering
...
Figure~\ref{fig:dist_other_data} shows the distribution of age, height, weight, and running mean outdoor temperature grouped by sex.
...
% todo: check that the colours are distinguishable for color blind and change them since I am using the same color palette to differentiate PMV and PMVCE; same comments as previous chart; use a boxen plot and have 4 for men and 4 for women. Tormt is missing the unit; age unot should be year. This figure can be merged with figure 2 because it is a visualization of the database. I would use blue (HEX: 003262, RGB (0, 50, 98)) and gold (HEX: FDB515, RGB (253, 181, 21)) as color because blue and green could be confounded by colorblind people. To be more consisten in the paper, I would use the letter value plots for this and the figure in section 3.3.2
\begin{figure*}[htb!]
    \centering
...
% todo Use the words, not the numbers. Please use the color pallet from Toby's paper. It is hard to notice that these are two distint graph because the legend is moving accross. I suggest to move the legend above and have a thin grey line to separate the two group, another option would be to add a light grey background for the two PMV and show that they are distinct. 
% todo Move the percentage, on top and move it horizontally.
% todo remove the white space between the boxes (horizontal lines)
% todo TSV use words not numbers
% todo Move this also above the figure, over each column. I would also add the term. Overall accuracy  PMV = 33%
\begin{figure*}[htb!]
    \centering
...
% todo in the table I should also report the F1 scores if I filter out some of the TSV. The PMV model should only be used in the range -2 and 2 so at least we should report also the results in this range.
\begin{table}[htb!]
    \centering
    \input{tables/f1.tex}
...
% todo I would not use grey for the data, it is too similar to the gridlines. A light color is fine, light brown (depending on the color pallet used in the rest of the paper) could work. The regression line could be done even thicker. Be more explicit in the graph of what is measured and what is calculated. The Y axis should be labeled as PMV
...
% todo You need to check of the four distibution are normally distributed, if so, report mean and SD, if not report median and IQR, no need to report all of them. I guess that Pswetest is an indication of this, so, for the entire dataset it seems that it is not sufficiently normally distributed, so report only median and IQR. The Pswetest This index is not reported, explained, and interpreted in the paper. I would like to have these figure a bit longer in the Y-axis. they are kind of squized. The word "delta" does not mean difference. So, I suggest to use "diffference" or just show the equation Bias=PMVi-TSVi. 1) Can this chart be transformed in a letter value plot? The Caption is missing
\begin{figure*}[htb!]
    \centering

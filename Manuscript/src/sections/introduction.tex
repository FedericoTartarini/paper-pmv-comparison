...
\todo[inline, color=blue!40]{
    FT. Firstly, it is important to note that the PMV does not aim to estimate sweating. 
    While this is a limitation, neither the PMV nor the PMV CE models include this capability.  
    The Std 55 did not modify the PMV heat balance equation but instead reduced the air and mean radiant temperatures by the cooling effect, assuming this adequately accounts for sweating. 
    However, this approach does not equate to an accurate heat balance calculation where sweat rate is explicitly modeled, as seen in the Gagge two-node model.  
    Consequently, the PMV CE does not: 1) use the Gagge two-node model to compute convective and evaporative heat loss from the body, nor 2) explicitly model physiological cooling in warm conditions where sweat evaporation becomes a key source of heat loss.  
    Secondly, in the PMV CE model, the cooling effect is applied in all conditions, not just in warm ones.  
    Thirdly, the definition of SET can also be somewhat unclear. 
    It is not calculated for a reference environment with standardised clothing and metabolic rates. 
    Instead, it represents the temperature of a hypothetical isothermal environment at 50\% RH, $<$0.1 m/s (20 fpm) average air velocity, where the total heat loss from the skin of an imaginary occupant (wearing clothing standardised for the activity) equals the heat loss from a person in the actual environment with their actual clothing and activity level.  
    Finally, as you mentioned, \textit{The reduced temperature is entered into the PMV heat balance model in a still air condition, from which the PMV index is calculated.} However, this does not guarantee that the person will not feel hot or sweat in this condition and, therefore, does not address the inherent issues of the original PMV model.
}
...
\todo[inline]{EA. true enough, but in the standard, the objective is to have a rational physiology model.
...
\todo[inline, color=blue!40]{FT. The PMV CE and the Gagge model are different, hence, publications about the latter do not sufficiently describe the rationale of the new model, moreover, the accuracy of the PMV CE was not tested}
...
\todo[color=blue!40]{FT. In the manuscript, we decided not to include a Figure for 0.6m/s since in the ASHRAE DB 2 we hardly reach that value.
I am happy to generate a figure for you if need be}
\begin{figure}[!htb]
    \centering
...
\begin{enumerate}
    ...
    You are citing skin wettedness values that have not been introduced in this paper (and are not part of any standard). 
    Where did they come from? You allude to results from Huang 2014 that the reader is supposed to know about. 
    The skin-wettedness discussion is irrelevant unless you go into a lot more physiological detail here.}
    \todo[inline, color=blue!40]{FT. We developed this thought-example to demonstrate that while the PMVCE seeks to improve the modeling of sweat rates, it does not alter the underlying PMV equations. 
    As a result, there are situations where the PMVCE may still fail to provide accurate predictions.
    We have modified the example and changed the air speed from 0.15 to 0.25 m/s}
    ...
    \todo[inline, color=blue!40]{FT. I agree that this may or may not be significant, depending on the context and the computation speed could be improved.}
\end{enumerate}

...
% todo v_95_perc_max these variables should be calculated dropping nan
...
For both models the simple accuracy decreases when we only include entries with \ac{v}~$\geq$~\qty{0.2}{\m\per\s}.
...
For the subset of data with \ac{v}~$\geq$~\qty{0.2}{\m\per\s} the F1-scores of the \ac{pmv-ce} are at best \num{0.01} point higher than \ac{pmv} which does not justify its superiority.
On the contrary the F1-macro score of the \ac{pmv} is \num{.23}, \qty{44}{\percent} higher than the \ac{pmv-ce} which is \num{.16} for subset of data with $\lvert \textrm{PMV}\lvert \leq 1.5$ and $\lvert \textrm{TSV}\lvert \leq 1.5$.
These results imply that the \ac{pmv} is more accurate than the \ac{pmv-ce} within the range in which both model perform better, i.e., closer to thermo neutrality.
\begin{table}[htb!]
    \centering
    \input{tables/f1.tex}
...
Both \ac{pmv} formulations always under-predicted the thermal sensation of people who reported to be on the extremes of the \ac{tsv}; the slope of the curve was significantly lower than 1.
...
% todo in the plot do not report the mean value but the range instead since for example V max is 0.46m/s
% todo report p-values
\begin{figure*}[htb!]
    \centering
...
Both model had a median bias higher or equal to $\lvert0.5\lvert$ for \ac{tdb}~$\leq$~\qty{20}{\celsius}; \ac{clo}~$\leq$~\qty{.6}{clo} and $\geq$~\qty{1.2}{clo}.
Surprisingly again the bias of the \ac{pmv-ce} model was significantly worse than the one of the \ac{pmv} for \ac{v}~$\geq$~\qty{0.2}{\m\per\s}.
% todo report the p-value

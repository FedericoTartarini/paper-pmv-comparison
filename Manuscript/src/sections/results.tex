...
The \ac{pmv} and \ac{pmv-ce} only differ when the value of \ac{v}~$\geq$~\qty{0.1}{\m\per\s}, however, the median value in the \ac{db2} was \var{v_median} suggesting that most of the data points were collected in buildings with low air movement.
Consequently, we also could not reliably test the accuracy of the \ac{pmv} formulations for values of \ac{v}~$\geq$~\var{v_95_perc_max}.
...
% todo add results to supplementary material and reference to section in the supplementary material
...
%todo add reference to the supplementary material

...
\todo{EA. But this statement is incorrect for ASHRAE Std 55, which only defines its single comfort zone using +/-0.5}
\todo[inline, color=blue!40]{FT. We clarified this issue above.}
...
\todo{unclear what limits you are talking about, or who the audience is for your suggestions.  The ComfortTool allows extrapolation to high PMVs, but that is not true for ASHRAE 55 itself.  
Your suggestions are just aimed at you as keepers of the ComfortTool}
\todo[inline, color=blue!40]{FT. The limits are the one described in Table 2, for example, we suggest to use the model only from 0.3 and 1.5 clo and met between 1 and 2.}
...
\todo{EA. This is a major suggestion with very significant implications for energy etc that have not been discussed in the paper.  There are surely reasons people have not used that scale on its own, for this purpose.}
\todo[inline, color=blue!40]{FT. We discussed this in Figure 5a and in its related text. The PMV model assume that people are comfortable when they claim to be slightly warm or slightly cool, this is explicit in Fanger's book.}

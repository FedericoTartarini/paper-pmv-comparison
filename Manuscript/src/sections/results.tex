...
\subsection{Comparison between the PMV formulations with the two-node model}\label{subsec:comparison-between-the-pmv-formulations-wih-the-two-node-model} % todo Stefano Schiavon: call it Gagge PMV model in line with the text below
% todo consider moving to appendix and explain well why I have done this. Stefano Schiavon: The logic and need of this paragraph needs to be better explained. Why did you add? Why can we assume that the PMV-SET is better? Who developed the PMV-SET? We should discuss if we need this paragraph and if it add something that is important and not reported in other sections. In other to do this, we need to explain why we think the PMV-SET is important even if it is not present in standards.

We compared how the results of the \ac{pmv} and \ac{pmv-ce} perform against the results of the two-node model because xxxx. % todo provide information
% todo: Stefano Schiavon: Please provide more details of how these inputs were generated, within which ranges, etc.
...
% todo Stefano Schiavon: Beside the visualization, can you discuss here also the quantitative results to support the conclusion?
The results of the \ac{pmv} model had a higher agreement than those obtained with the \ac{pmv-ce} model for \ac{pmvg} $\geq$ \qty{0.5}{\m\per\s}. % todo Stefano Schiavon: This is unclear and cannot be deduced from the figure, from where does it come from? How can a PMV model be described by an air speed?
...
% todo Stefano Schiavon: Please add references suportting this.
...
% todo talk also about how sweat is calculated
...
% todo Stefano Schiavon: This is vague, Which papers or data is available to update that equation/mapping?
...
% todo we need to check this since we may only recommend the use of the PMV model for thermal neutrality
We showed that people tend to be closer to neutrality than what the models predicted (see Figure XXX), this is an indication that they were exposed to compensable thermal conditions. % todo add figure number
...
% todo Stefano Schiavon: The difference between these two points ii) and iii) are not clear, can you add more explanation?

...
Here we are showing the results for the subset of data with \ac{v} measured at three heights and \ac{tdb}~$\geq$~\qty{24}{\celsius}.
We are also reporting these results since the \ac{pmv-ce} aims to more accurately predict the thermal sensation, compared to the \ac{pmv}, when air movement is used to cool the occupants.
...
This result is consistent with the previous analysis and confirms that the \ac{pmv} model is more accurate than the \ac{pmv-ce}.